...
func (a *Auth) signup(c *gin.Context) {
	...
	fileByte, err := ioutil.ReadFile("private_key.pem")
	if err != nil {
		apperr.Response(c, apperr.New(http.StatusUnauthorized, "Invalid password format1."))
		return
	}
	priv_parsed, err := ParseRsaPrivateKeyFromPemStr(string(fileByte))
	if err != nil {
		apperr.Response(c, apperr.New(http.StatusUnauthorized, "Invalid password format2."))
		return
	}
	decoded, err := base64.StdEncoding.DecodeString(e.Password)
	if err != nil {
		apperr.Response(c, apperr.New(http.StatusUnauthorized, "Invalid password format3."))
		return
	}
	password, err := rsa.DecryptPKCS1v15(rand.Reader, priv_parsed, decoded)
	if err != nil {
		apperr.Response(c, apperr.New(http.StatusUnauthorized, "Invalid password format4."))
		return
	}
...
func (a *Auth) recoverPassword(c *gin.Context) {
	...
	fileByte, err := ioutil.ReadFile("private_key.pem")
	if err != nil {
		apperr.Response(c, apperr.New(http.StatusUnauthorized, "Invalid password format5."))
		return
	}
	priv_parsed, err := ParseRsaPrivateKeyFromPemStr(string(fileByte))
	if err != nil {
		apperr.Response(c, apperr.New(http.StatusUnauthorized, "Invalid password format6."))
		return
	}
